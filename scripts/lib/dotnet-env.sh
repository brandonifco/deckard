...
#   DECKARD_DOTNET_SOURCE   "repo-local" | "primary-checkout" | "path"
#   DECKARD_DOTNET_HOME     the .dotnet directory used, empty when DECKARD_DOTNET_SOURCE=path
#
...
_deckard_repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

DECKARD_DOTNET_SOURCE="path"
DECKARD_DOTNET_HOME=""

if [[ -d "$_deckard_repo_root/.dotnet" ]]; then
  DECKARD_DOTNET_SOURCE="repo-local"
  DECKARD_DOTNET_HOME="$_deckard_repo_root/.dotnet"
else
  _deckard_primary_root="$(deckard_primary_checkout_root "$_deckard_repo_root")" || _deckard_primary_root=""
  if [[ -n "$_deckard_primary_root" && -d "$_deckard_primary_root/.dotnet" ]]; then
    DECKARD_DOTNET_SOURCE="primary-checkout"
    DECKARD_DOTNET_HOME="$_deckard_primary_root/.dotnet"
  fi
fi

if [[ "$DECKARD_DOTNET_SOURCE" != "path" ]]; then
  export DOTNET_ROOT="$DECKARD_DOTNET_HOME"
  export PATH="$DECKARD_DOTNET_HOME:$PATH"
fi

unset _deckard_repo_root _deckard_primary_root
