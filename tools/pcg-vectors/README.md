...
`tests/Deckard.Core.Tests/Randomness/Pcg32ReferenceVectors.cs` is generated, not written.
...
Deckard's replay contract is the PCG32 sequence itself. A wrong vector does not fail —
...
recipe are what Deckard actually needs. Nothing under `tools/pcg-vectors/` is copied from
upstream.
