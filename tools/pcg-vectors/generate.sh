...
#
# The vectors that pin Deckard's replay contract must come from the reference
...
# committing it would make Deckard a redistributor for no benefit. What this repository
...
FIXTURE="$REPO_ROOT/tests/Deckard.Core.Tests/Randomness/Pcg32ReferenceVectors.cs"
...
if [[ "$MODE" == "--check" ]]; then
  if diff -u "$FIXTURE" "$TARGET"; then
    ...
    die "the committed fixture does not match a fresh run against the reference.
       Replacing it changes the replay contract -- see ADR 0002."
  fi
else
